...
// This code dispatch into an action creator
...
```js
// The constants for Action Types
const LOGIN = 'LOGIN';
const LOGOUT = 'LOGOUT';
...
// A callback function that increments count.
const callback = () => {
  count++;
}
...
// Combining multiple reducers like this:
const rootReducer = Redux.combineReducers({
...
So far these actions have not contained any information other than a `type`. You can also send specific data along with your actions. This is very common because actions usually originate from some user interaction and tend to carry some data with them. The Redux store often needs to know about this data.
```js
...
// Dispatch action data to store
store.dispatch(addNoteText('Hello!'));
console.log(store.getState());
```
---

## 12. Use Middleware to Handle Async Actions
So far these challenges have avoided discussing asynchronous actions, but they are an unavoidable part of web development. At some point you'll need to call asynchronous endpoints in your Redux app, so how do you handle these types of requests? Redux provides middleware designed specifically for this purpose, called Redux Thunk middleware. Here's a brief description how to use this with Redux.

To include Redux Thunk middleware, you pass it as an argument to `Redux.applyMiddleware()`. This statement is then provided as a second optional parameter to the `createStore()` function. Take a look at the bottom code below to see this. Then, to create an asynchronous action, you return a function in the action creator that takes `dispatch` as an argument. Within this function, you can dispatch actions and perform asynchronous requests.
...
const handleAsync = () => {
  return function(dispatch) {
    ...
    store.dispatch(requestingData(dispatch));
...
This code is the templare of creating a counter:
```js
...
Resultant code:
```js
// Defining constant action types
const INCREMENT = 'INCREMENT'; 
const DECREMENT = 'DECREMENT';

// Defining Reducer function; usually have switch to check states.
const counterReducer = (state = 0, action) => {
  switch (action.type) {
    case INCREMENT:
    return state + 1;
...
const immutableReducer = (state = todos, action) => {
  switch(action.type) {
    case ADD_TO_DO:
      ...
      // One of the ways to append array without mutating
...
```
[Useful references](https://daveceddia.com/react-redux-immutability-guide/#redux-prepend-an-item-to-an-array)

---

## 15. Use Spread Operator on Arrays
...
## 16. Removing Item from an Array
